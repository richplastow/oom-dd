...
Description: The Developing Dreams VR Website
...
$oomClassesPath = dirname(__FILE__) . '/../../../dist/php/oom-dd.7.php';
...
if (! $debug) {
    ...
    register_activation_hook( __FILE__, 'oom_dd_on_activation' );
    function oom_dd_flush_rewrites() {
        oom_dd_register_cpts_and_cmb2s();
        flush_rewrite_rules();
    }
    ...
    add_action( 'cmb2_render_text_number', 'oom_dd_cmb2_render_text_number', 10, 5 );
    function oom_dd_cmb2_render_text_number($fa, $ev, $oi, $ot, $field_type_object) {
        echo $field_type_object->input( array('type' => 'number', 'step' => 'any') );
    }
    add_filter( 'cmb2_validate_text_number', 'oom_dd_cmb2_validate_text_number', 10, 2 );
    function oom_dd_cmb2_validate_text_number($override_value, $value) {
        if (! is_numeric($value) ) $value = ''; // not a number? empty the value
        return $value;
    }
    add_action( 'cmb2_render_text_nnint', 'oom_dd_cmb2_render_text_nnint', 10, 5 );
    function oom_dd_cmb2_render_text_nnint($fa, $ev, $oi, $ot, $field_type_object) {
        echo $field_type_object->input( array('type' => 'number', 'min' => '0') );
    }
    add_filter( 'cmb2_validate_text_nnint', 'oom_dd_cmb2_validate_text_nnint', 10, 2 );
    function oom_dd_cmb2_validate_text_nnint($override_value, $value) {
    ...
    add_action('cmb2_init', 'oom_dd_register_cpts_and_cmb2s');

    //// Define CPTs and CMB2s.
    function oom_dd_register_cpts_and_cmb2s () {
        global $oomClasses;
    ...
    function oom_dd_get_cm($object, $field_name, $request, $object_type) {
        return get_post_meta($object['id'], $field_name, true); //@TODO $object->ID ?
    }

    //// Writes custom meta for use in the REST API.
    function oom_dd_update_cm($value, $object, $field_name, $request, $object_type) {
    ...
    add_action('rest_api_init', 'oom_dd_register_rest_cm');

    //// Register custom meta with REST API.
    function oom_dd_register_rest_cm () {
        global $oomClasses;
        ...
        foreach ($oomClasses as $classname => $class) {
            ...
            foreach ($schema['attr'] as $attrname => $desc) {
                ...
                register_rest_field($nameLCU, $attrnameLCU, array(
                    'get_callback'    => 'oom_dd_get_cm'
                  , 'update_callback' => 'oom_dd_update_cm'
                  , 'schema'          => null
                ) );
            }//foreach attr
