...
describe('The Oom.Dd.Cloud.devMainVue() component', function (done) {
    const
        testID = 'test-oom-dd-cloud-devmainvue' // also used for component tag
      , vueComponent = Vue.component( testID, Class.devMainVue(instance) )
      , $container = $('.container').append(`<div class="row ${hid?'hid':''}" `
          + `id="${testID}"><${testID}>Loading...</${testID}></div>`)
      , vue = new Vue({ el:'#'+testID, mounted:testAfterMounted })
    ...
    //// Oom.Dd.Cloud.devMainVue(): The component itself.
    it('is a viable Vue component', function(){try{
        eq( $('#'+testID).length, 1
          , '#'+testID+' exists' )
        eq( $('#'+testID+' .dev-main').length, 1
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic statics - initial values.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('shows correct initial statics', function (done) {
        Vue.nextTick((function(){let error;try{
            for (let key in stat) {
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-only statics - shows changes.
    it('shows that read-only statics have changed', function (done) {
        const cache = { good:{} }
        for (let key in stat) {
            if (! isReadOnly(key) ) continue // only read-only properties
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write statics - shows changes.
    it('shows that read-write statics have changed', function (done) {
        const cache = { good:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue // only read-write properties
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write statics - valid input.
    it('updates read-write statics after UI input', function (done) {
        const cache = { $el:{}, good:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write statics - invalid input.
    it('does not update read-write statics after invalid UI input', function (done) {
        const cache = { $el:{}, orig:{} }
        for (let key in stat) {
            if (! isReadWrite(key) ) continue
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic attributes - initial values.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('shows correct initial attributes', function (done) {
        Vue.nextTick((function(){let error;try{
            for (let key in attr) {
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-only attributes - shows changes.
    it('shows that read-only attributes have changed', function (done) {
        const cache = { good:{} }
        for (let key in attr) {
            if (! isReadOnly(key) ) continue // only read-only properties
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write attributes - shows changes.
    it('shows that read-write attributes have changed', function (done) {
        const cache = { good:{} }
        for (let key in attr) {
            if (! isReadWrite(key) ) continue // only read-write properties
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write attributes - valid input.
    it('updates read-write attributes after UI input', function (done) {
        const cache = { $el:{}, good:{} }
        for (let key in attr) {
            if (! isReadWrite(key) ) continue
    ...
    //// Oom.Dd.Cloud.devMainVue(): Automatic read-write attributes - invalid input.
    it('does not update read-write attributes after invalid UI input', function (done) {
        const cache = { $el:{}, orig:{} }
        for (let key in attr) {
            if (! isReadWrite(key) ) continue
...
})//describe('The Oom.Dd.Cloud.devMainVue() component')
...
describe('The Oom.Dd.Cloud.devThumbAFrame*() set', function (done) {
    ...
    //// Oom.Dd.Cloud.devThumbAFrameVue(): `hilite` static and attribute - change.
    //// `Vue.nextTick()` because Vue hasn’t initialised the properties yet.
    it('changing `stat/attr.hilite` changes box color', function (done) {
        const { firstObj, firstHex, secondObj, secondHex } = generateRandomColors()
        stat.hilite = firstHex
    ...
    //// Oom.Dd.Cloud.devThumbAFrameVue(): boxes respond to click.
    it('boxes can change `stat/attr.hilite` on click', function (done) {
        const { thirdObj, thirdHex, fourthObj, fourthHex } = generateRandomColors()
        const onOomEvent = function (evt) {
            if (! evt.detail) return
...
})//describe('The Oom.Dd.Cloud.devThumbAFrameVue() component')
